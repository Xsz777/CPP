%demo
    C = 10;%预设目标点个数
    DX = 5;
    detect_radius = 5;%检测半径
    L = 60;%机器人电线长度
    H = 20;
    dx = 4;
    work_radius = 2;%工作半径
    search_radius = 3;%搜寻半径
    fps = 10;%刷新帧数
    z_submarine =98;%模拟海底的高度
    z_cruising =100;%巡航平面的高度
    max_deceleration = 5;  %减速时的加速度
    transl_spd = 1.5;%步长
    rot_spd = pi;

    Delta_y = 0.5;%逼近过程中水平上单位移动距离
    Delta_z = 0.5;%逼近过程中竖直上单位移动距离
    down_work_radius = 0.5;%坐滩之后机器人工作手的长度
    down_transl_spd = 0.1;%逼近过程中的步长
    down_transl = 4;%垂直下潜阈值
        ...
        while j <= size(x, 1)
            ...
            if b > 1/fps                  % objects 就是目标点
                ...
                if has_object    %find
                    ...
                    orientation = rotate(orientation, calc_orientation(stop_position(1:2), position_obj(1:2)), rot_spd);%转动角度
                    position1 = dive(stop_position,position_obj,down_transl_spd,an,fps);%巡航平面上移动，直至与目标水平共线
                    position2 = approach(position1 ,position_obj,an,down_transl_spd,fps,Delta_y,Delta_z,down_transl)%逼近目标（竖直，水平）
                    position = position2;
                    [position, orientation, objects] = greedy_grab([position orientation], objects, work_radius, search_radius, transl_spd, rot_spd, an, fps);%海底贪心，寻找更多目标
                    orientation = rotate(orientation, calc_orientation(position(1:2), stop_position(1:2)), rot_spd);
                    position = float(position, stop_position, transl_spd, an, fps,down_transl_spd,z_cruising);%上浮到下潜前位置
